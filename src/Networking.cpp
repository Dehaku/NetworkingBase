...
void OldDealPackets()
{
    ...
    //std::cout << "PL: " << PackLimit << std::endl;
    ...
    for(int i = 0; i != PackLimit; i++)
    {
        ...
        if(GotIdent != "")
        {
            ...
            if(GotIdent == ident.connection)
            {
                ...
                std::cout << "Peer connected; " + peer.name << std::endl;
...
void OldTcpServFirstRun(unsigned short port)
{
    ...
    {
        throw std::runtime_error("Failed to connect to main port!");
    }
    ...
    while(selector.wait())
    {
        ...
        if(selector.isReady(servListener))
        {
            ...
            if (servListener.accept(*client) == sf::Socket::Done)
            {
                ...
                sf::Packet packy;
                packy << ident.connectionSuccessful;
                clients.back()->send(packy);
            }
            else
            {
...
void OldTcpCliFirstRun(unsigned short port)
{
    ...
    if (cliSocket.connect("127.0.0.1", network::mainPort) != sf::Socket::Done)
    {
        throw std::runtime_error("Failed to connect to server!");
    }

    if(network::name == "")
        network::name = std::to_string(random(1,1000));
